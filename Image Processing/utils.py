...
def apply_filter(image, fltr, anchor_filter=(0, 0), pad_mode="zeros"):
    ...
    pad_width = (
        (top_fltr_width, bottom_fltr_width),
        (left_fltr_width, right_fltr_width),
    )
    ...
    for i in range(top_fltr_width, n_rows - bottom_fltr_width):
        for j in range(left_fltr_width, n_cols - right_fltr_width):
            overlap = padded_img[
                (i - top_fltr_width) : (i + bottom_fltr_width + 1),
                (j - left_fltr_width) : (j + right_fltr_width + 1),
            ]
            filtered_image[i - top_fltr_width, j - left_fltr_width] = abs(
                np.sum(fltr * overlap)
            )
